...
public class ResponderQM implements Parcelable
{
    ...
    private static final String TAG = "ResponderQM";
    private static final String SRC_PKG = "edu.umich.flowfence.study.presencebasedcontrol";
    ...
    public static void pollPresenceAndCompute()
    {
        try {
            ...
            SharedPreferences presencePrefs = FlowfenceContext.getInstance().createPackageContext(SRC_PKG, 0).getSharedPreferences(STORE_NAME, Context.MODE_WORLD_READABLE);
            ...
            if(!history.equals(presence)) {
                ...
                if (op != null) {
                    ISmartSwitchAPI switchAPI = (ISmartSwitchAPI) FlowfenceContext.getInstance().getTrustedAPI("smartswitch");
    ...
    public static void putHistory(String hist)
    {
        ...
        SharedPreferences myprefs = FlowfenceContext.getInstance().getSharedPreferences("hist_store", Context.MODE_PRIVATE);
    ...
    {
        SharedPreferences myprefs = FlowfenceContext.getInstance().getSharedPreferences("hist_store", Context.MODE_WORLD_READABLE);
    ...
    public static final Parcelable.Creator<ResponderQM> CREATOR = new Parcelable.Creator<ResponderQM>()
    {
        public ResponderQM createFromParcel(Parcel in) {
            return new ResponderQM(in);
        }

        public ResponderQM[] newArray(int size) {
            return new ResponderQM[size];
        }
    };

    private ResponderQM(Parcel in)
    {
